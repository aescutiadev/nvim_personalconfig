...
local function neo_tree_is_open(position)
  for _, win in ipairs(vim.api.nvim_list_wins()) do
    ...
    if vim.bo[buf].filetype == "neo-tree" then
      ...
      if position == "float" and cfg.relative ~= "" then
        return true
      end

      if position == "left" and cfg.relative == "" then
        return true
      end
    end
  end
...
local function toggle_neotree(position)
  local other = position == "left" and "float" or "left"

  if neo_tree_is_open(position) then
    vim.cmd(("Neotree action=close position=%s"):format(position))
    return
  end

  pcall(vim.cmd, ("Neotree action=close position=%s"):format(other))
  vim.cmd(("Neotree position=%s"):format(position))
end
...
Snacks.keymap.set("n", "<leader>e", function()
  toggle_neotree("left")
end, { desc = "Toggle Neo-tree Left" })

Snacks.keymap.set("n", "<leader><leader>", function()
  toggle_neotree("float")
